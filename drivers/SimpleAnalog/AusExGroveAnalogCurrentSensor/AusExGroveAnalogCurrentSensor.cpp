...
bool _TEMPLATE_CLASS::getEvent(sensors_event_t* event){
  ...
  _SENSOR_VALUE_TYPE sensorMax=0,sensorValue=0;
  while((millis()-start_time) <_SENSOR_MIN_DELAY_MILLI) {
  ...
  event->_SENSOR_RETURN_VALUE = calcValue(sensorMax);
  return true;
}
...
_SENSOR_VALUE_TYPE _TEMPLATE_CLASS::calcValue(int val){
  return (float)val/1024*_vdd/800*2000000/1.414;
}
